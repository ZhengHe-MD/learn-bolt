...
事务是数据库提供给应用程序的一层抽象，这层抽象将所有并发控制、软硬件各种异常信息隐藏，只对应用暴露出两种状态：成功(commit)和终止(abort)。读写事务中的数据修改要么执行完成、要么回滚到修改前的状态，不存在执行到一半的中间态。有了事务支持，应用遇到执行失败时按需重试即可。Bolt 提供**可序列化(serializable)的、满足 ACID** 的事务支持，本节就来讨论一下 Bolt 的相关实现。
